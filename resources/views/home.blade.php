  ...
  <section class="py-16 bg-white border-b border-gray-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <!-- Gambar Jasa -->
          <div class="relative">
            <div class="w-full h-40 bg-gray-100 flex items-center justify-center text-gray-400 text-sm font-medium">
              GAMBAR JASA
            </div>
            <div class="absolute top-3 right-3 bg-white text-yellow-500 text-sm font-bold px-2.5 py-1 rounded-lg flex items-center shadow border border-gray-100">
          ...
          <!-- Info -->
          <a href="{{ route('layanan.show', $fl['id_layanan'] ?? 1) }}" class="block p-4">
              <h3 class="font-bold text-dark text-base mb-0.5">{{ $fl['nama_jasa'] ?? 'JUDUL JASA' }}</h3>
              <p class="text-gray-500 text-sm mb-1">{{ $fl['nama_pengguna'] }}</p>
              <p class="text-accent font-bold text-base mb-3">Rp {{ number_format($fl['harga'] ?? 0, 0, ',', '.') }}/satuan</p>
              <div class="flex items-center text-gray-500 text-xs">
                  <svg class="w-3.5 h-3.5 mr-1 text-accent" fill="currentColor" viewBox="0 0 24 24">
