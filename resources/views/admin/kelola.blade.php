    ...
    <div class="lg:col-span-2 flex flex-col gap-6">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tr class="hover:bg-gray-50/50 transition-colors">
                ...
                <td class="p-4">
                    <span class="px-3 py-1 bg-gray-100 text-gray-600 border border-gray-200 rounded-full text-[11px] font-bold inline-block">{{ $kat['jumlah_jasa'] ?? 0 }} Jasa</span>
                </td>
                ...
                <div class="flex items-center justify-end gap-2">
                    ...
                    <form method="POST" action="{{ route('admin.kelola.kategori.destroy', $kat['id_kategori']) }}" class="inline-block" onsubmit="return confirmAction(event, 'Apakah Anda yakin ingin menghapus kategori ini?');">
                        @csrf
                        @method('DELETE')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="3" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-accent focus:bg-white text-sm text-dark font-medium"></textarea>
        </div>
...
<div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0" id="modalEditSatuanContent">
    ...
    <form id="formEditSatuan" method="POST" action="" class="p-6 space-y-4">
        ...
        <div>
            <label class="block text-sm font-bold text-dark mb-1.5">Deskripsi</label>
            <textarea id="edit_deskripsi_satuan" name="deskripsi" rows="3" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-accent focus:bg-white text-sm text-dark font-medium"></textarea>
        </div>
        <div class="flex gap-3 pt-4">
...
<!-- Modal Edit Kategori -->
<div id="modalEditKategori" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-all duration-300">
<div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0" id="modalEditKategoriContent">
    <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
    <h3 class="font-bold text-dark text-lg">Edit Kategori</h3>
    <button onclick="closeModal('modalEditKategori')" class="p-2 text-gray-400 hover:text-dark hover:bg-gray-100 rounded-lg transition-colors">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
    </button>
    </div>
    <form id="formEditKategori" method="POST" action="" class="p-6 space-y-4">
        ...
        <div class="flex gap-3 pt-4">
            <button type="button" onclick="closeModal('modalEditKategori')" class="flex-1 py-2.5 text-center text-sm font-bold text-gray-500 hover:bg-gray-50 rounded-xl border border-gray-200 transition-colors">Batal</button>
            <button type="submit" class="flex-1 py-2.5 bg-accent text-white text-sm font-bold rounded-xl shadow-md hover:bg-orange-700 transition-colors cursor-pointer">Perbarui</button>
        </div>
    </form>
</div>
...
function openEditJasaModal(js) {
    ...
    openModal('modalEditJasa');
}

function openEditSatuanModal(st) {
    document.getElementById('edit_nama_satuan').value = st.nama_satuan;
    document.getElementById('edit_deskripsi_satuan').value = st.deskripsi || '';
